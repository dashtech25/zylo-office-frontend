...
import type { Station, StationCurrentState, Tank } from "@/core/api/zyloLiquid";
        ...
        <table className="w-full border-collapse text-body-sm">
          ...
          <tbody>
            {groups.map((group) => {
              ...
              const stationRate = stationCapacity > 0 ? Math.round((stationVolume / stationCapacity) * 100) : 0;
              ...
              return (
                <Fragment key={group.station.id}>
                  ...
                  {group.rows.map((row) => {
                    const rate = row.capacityLiters > 0 ? Math.round((row.volumeLiters / row.capacityLiters) * 100) : 0;
                    return (
                  ...
                  <tr className="border-b border-border-subtle bg-surface-muted/60 font-medium">
                    ...
                    <td className="py-2 pl-4 tabular-nums text-text-muted">{stationRate}%</td>
                    <td className="py-2 text-right tabular-nums text-text">
            ...
            <tr className="bg-primary-muted font-bold text-primary">
              ...
              <td className="py-3 pl-4 tabular-nums">{totalCapacity > 0 ? Math.round((totalVolume / totalCapacity) * 100) : 0}%</td>
