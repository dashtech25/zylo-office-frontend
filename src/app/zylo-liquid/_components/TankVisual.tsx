...
function GaugeVertical({ data, big, fuelColor }: { data: TankVisualData; big?: boolean; fuelColor: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} width={w} height={h} role="img" aria-label={`Jauge, remplissage ${pct.toFixed(1)}%`}>
      ...
      <text x={bx + bw / 2} y={h - 4} fontSize={10} textAnchor="middle" fontWeight={700} fill="var(--color-text)" fontFamily="ui-monospace,monospace">
        {data.pct === null ? "—" : `${pct.toFixed(1)} %`}
      </text>
    </svg>
  );
}

function GaugeHorizontal({ data, big, fuelColor, tank }: { data: TankVisualData; big?: boolean; fuelColor: string; tank: Tank }) {
  const w = 210;
  const h = big ? 128 : 104;
  const bx = 8,
    by = 10,
    ...
    bh = h - 32,
    r = bh / 2;
  const tankHeight = data.tankHeightMm;
  const hr = tankHeight > 0 && data.heightMm !== null ? clampPct((data.heightMm / tankHeight) * 100) : null;
  const waterPct = tankHeight > 0 ? clampPct((data.waterHeightMm / tankHeight) * 100) : 0;
  const col = data.pct === null ? "var(--color-text-disabled)" : data.toneColor === "var(--color-success)" ? fuelColor : data.toneColor;
  const yFor = (mm: number) => (tankHeight > 0 ? by + bh - (bh * clampPct((mm / tankHeight) * 100)) / 100 : by + bh);
  const yLow = yFor(tank.lowAlarmMm);
  const yHigh = yFor(tank.heightAlarmMm);

  return (
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} preserveAspectRatio="xMidYMid meet" role="img" aria-label="Cuve horizontale, niveau physique">
      <rect x={bx} y={by} width={bw} height={bh} rx={r} fill={ULLAGE_COLOR} stroke={ULLAGE_BORDER} />
      <clipPath id={`ch-${h}`}>
      ...
      <g clipPath={`url(#ch-${h})`}>
        {hr !== null && (
          <>
            <rect x={bx} y={by + bh - (bh * hr) / 100} width={bw} height={(bh * hr) / 100} fill={col} opacity={0.88} />
            {data.waterHeightMm > 0 && (
              <rect x={bx} y={by + bh - Math.max(3, (bh * waterPct) / 100)} width={bw} height={Math.max(3, (bh * waterPct) / 100)} fill={WATER_COLOR} />
            )}
          </>
        )}
      </g>
      <line x1={bx} y1={yLow} x2={bx + bw} y2={yLow} stroke="var(--color-warning)" strokeWidth={2} strokeDasharray="5 3" />
      <line x1={bx} y1={yHigh} x2={bx + bw} y2={yHigh} stroke="var(--color-warning)" strokeWidth={2} strokeDasharray="5 3" />
      <ellipse cx={bx + r} cy={by + r} rx={r * 0.42} ry={r * 0.92} fill="none" stroke={ULLAGE_BORDER} />
      <text x={bx} y={h - 6} fontSize={10} fill="var(--color-text)" fontFamily="ui-monospace,monospace">
        {data.heightMm !== null ? `${Math.round(data.heightMm)} / ${Math.round(tankHeight)} mm` : "hauteur n/d"}
      </text>
      <text x={bx + bw} y={h - 6} fontSize={10} textAnchor="end" fontWeight={700} fill="var(--color-text)" fontFamily="ui-monospace,monospace">
        {data.volumeLiters === null ? "—" : `${Math.round(data.volumeLiters).toLocaleString()} L`}
      </text>
    </svg>
  );
}
...
export function TankVisual({ tank, state, mode, big, fuelColor }: { tank: Tank; state: TankCurrentState; mode: TankVisualMode; big?: boolean; fuelColor?: string | null }) {
  ...
  if (mode === "horizontal") return <GaugeHorizontal data={data} big={big} fuelColor={color} tank={tank} />;
...
export function TankFigures({ tank, state, showValue = true }: { tank: Tank; state: TankCurrentState; showValue?: boolean }) {
  ...
  const data = computeTankVisualData(tank, state);
  const fmt = (v: number) => `${Math.round(v).toLocaleString()} L`;
